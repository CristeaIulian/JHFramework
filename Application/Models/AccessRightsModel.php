...
/**
 * Access Rights Model Class.
 * 
*/
class AccessRightsModel extends DBModel {
	...
	*/
	public function getAll($limit = null, $filter = null) {
		...
		if (!is_null($filter) && isset($filter['AccessRights'])) {
			foreach ($filter['AccessRights'] as $key => $value) {
				$this->db->where($key . " LIKE '%" . $value . "%'");
			}
		}

		$this->db->order('controller, action');
